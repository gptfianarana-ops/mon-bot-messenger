...
function detecterIntention(texte) {
  ...
  const motsFonction = ['exercice', 'corrige', 'traduis', 'cv', 'profil', 'code', 'credit', 'defi', 'mémoire', 'memoire', 'redaction'];
  ...
  switch (etat.mode) {
    ...
    case 'admin_menu': {
      ...
      if (cmd === 'références' || cmd === 'references') {
        ...
        const list = JSON.parse(all);
        if (list.length === 0) {
          await sendMessage(senderId, '📚 Aucune référence enregistrée.', BOUTON_MENU);
        } else {
          let msg = '📚 **Liste des références :**\n\n';
          for (const id of list) {
            const doc = await redisGet(`reference:doc:${id}`);
            if (doc) {
              const data = JSON.parse(doc);
              msg += `- ${data.nom} (${data.segments.length} segments) - ${data.date.slice(0,10)}\n`;
            }
          }
      ...
      // Commande : supprimer_ref
      if (cmd.startsWith('supprimer_ref ')) {
        const id = cmd.replace('supprimer_ref ', '').trim();
        await redisSet(`reference:doc:${id}`, null);
        const allKey = 'reference:all';
        let all = await redisGet(allKey) || '[]';
        let list = JSON.parse(all).filter(x => x !== id);
        await redisSet(allKey, JSON.stringify(list));
        await sendMessage(senderId, `✅ Référence ${id} supprimée.`, BOUTON_MENU);
        return;
      }
      ...
      if (cmd === 'alerte') {
        await sendMessage(senderId, '🔔 Province des résultats :',
          [{ content_type:'text', title:'Antananarivo', payload:'ADMIN_ALERTE_antananarivo' }, { content_type:'text', title:'Fianarantsoa', payload:'ADMIN_ALERTE_fianarantsoa' }, { content_type:'text', title:'Toamasina', payload:'ADMIN_ALERTE_toamasina' }, { content_type:'text', title:'Mahajanga', payload:'ADMIN_ALERTE_mahajanga' }, { content_type:'text', title:'Antsiranana', payload:'ADMIN_ALERTE_antsiranana' }, { content_type:'text', title:'Vakinankaratra', payload:'ADMIN_ALERTE_vakinankaratra' }, { content_type:'text', title:'SAVA', payload:'ADMIN_ALERTE_sava' }]);
        return;
      }
