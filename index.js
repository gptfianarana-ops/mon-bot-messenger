// ============================================================
// index.js – BOT MESSENGER COMPLET (version intégrée)
// Toutes les fonctionnalités : personnalisation, gamification,
// recherche examens, correction, exercices, CV, simulateur Bac,
// Hianatra, crédits, alertes, graphiques, etc.
// ============================================================
...
// ============================================================
// GAMIFICATION : XP, NIVEAUX, BADGES, PROFIL, DÉFI QUOTIDIEN
// ============================================================
...
async function getStatsCounter(senderId, action) {
  const key = `stats:${senderId}:${action}`;
  const val = await redisGet(key);
  return val ? parseInt(val, 10) : 0;
}
async function incrementStatsCounter(senderId, action) {
  const key = `stats:${senderId}:${action}`;
  const actuel = await getStatsCounter(senderId, action);
...
async function ajouterXP(senderId, quantite, actionType) {
  ...
  if (actionType === 'correction') {
    await incrementStatsCounter(senderId, 'corrections');
    const count = await getStatsCounter(senderId, 'corrections');
...
// ============================================================
// MENU PRINCIPAL (Quick Replies) ET BOUTON RETOUR
// ============================================================
...
// ============================================================
// AUTRES FONCTIONS (CV, Simulateur Bac, Méthodologie, etc.) – copiées de votre code
// ============================================================
...
async function searchBepc(query, typeExam = 'bepc', tentative = 1) {
  ...
  try {
    ...
    return estTimeout ? "⏳ Le site officiel met trop de temps à répondre en ce moment (serveur lent ou surchargé). Réessaie dans quelques minutes.Maro ny mandefa message ka manasa anao hiverina afaka fotoana fohy" : 'Désolé, la recherche a échoué (le site est peut-être indisponible). Réessaie plus tard.';
  }
}
...
// ============================================================
// RECHERCHE BACCALAURÉAT (Multi-Provinces)
// ============================================================
const PROVINCE_MAP = {
...
const BACC_CONFIG = {
  fianarantsoa: { name: 'Fianarantsoa', type: 'api_json', baseUrl: 'https://bacc.univ-fianarantsoa.mg/api/search', endpoints: { nom: '/name/', mle: '/num/' } },
  antananarivo: { name: 'Antananarivo', type: 'api_json', baseUrl: 'https://tana-api.bacc.digital.gov.mg/api/search', endpoints: { nom: '/name/', mle: '/num/' } },
  toamasina: { name: 'Toamasina', type: 'api_json', baseUrl: 'https://toamasina-api.bacc.digital.gov.mg/api/search', endpoints: { nom: '/name/', mle: '/num/' } },
  mahajanga: { name: 'Mahajanga', type: 'api_json', baseUrl: 'https://mahajanga-api.bacc.digital.gov.mg/api/search', endpoints: { nom: '/name/', mle: '/num/' } },
  toliara: { name: 'Toliara', type: 'api_json', baseUrl: 'https://bacc.toliara.digital.gov.mg/api/search', endpoints: { nom: '/name/', mle: '/num/' } },
  antsiranana: { name: 'Antsiranana', type: 'api_json', baseUrl: 'https://diego-api.bacc.digital.gov.mg/api/search', endpoints: { nom: '/name/', mle: '/num/' } }
};
...
// ============================================================
// SYSTÈME D'ALERTES BACC
// ============================================================
const URL_PAGE_FACEBOOK = 'https://www.facebook.com/profile.php?id=100081570672160';
const MSG_INCITATION_ABONNEMENT = { fr: `📢 **INFO IMPORTANTE**\n\nPour recevoir nos alertes prioritaires et ne rien rater, assure-toi d'être abonné à notre page Tsarafandray Services !`, mg: `📢 **HO ANTSO :**\n\nMba hahazoanao ny fampandrenesana rehetra haingana dia haingana, manasa anao hanaraka (abonner) ny pejy Tsarafandray Services !` };
const MSG_PROPOSER_ALERTE = { fr: `🔔 Les résultats ne sont pas encore disponibles pour cette province.\n\nVeux-tu être alerté par message dès qu'ils seront publiés ?`, mg: `🔔 Mbola tsy vonona ny valim-panadinana ho an'ity faritany ity.\n\nTe hahazo fampandrenesana (notification) ve ianao raha vao mivoaka izany ?` };
...
// ============================================================
// TRACÉ DE COURBES (QuickChart.io)
// ============================================================
...
async function extraireFonctionGraphique(texte) {
  try {
    ...
    return { formule: data.formule, xMin: typeof data.xMin === 'number' ? data.xMin : -10, xMax: typeof data.xMax === 'number' ? data.xMax : 10 };
...
async function genererGraphiqueMath(formule, xMin, xMax) {
  try {
    ...
    if (nbPointsValides < nbPoints * 0.2) return null;
    const chartConfig = {
      type: 'line',
      data: {
        labels,
...
async function sendImage(recipientId, imageUrl) {
  ...
  } catch (err) { console.error('Erreur envoi image:', err.response?.data || err.message); }
}
...
async function sendFile(recipientId, fileUrl) {
  ...
  } catch (err) { console.error('Erreur envoi fichier:', err.response?.data || err.message); }
}

// ============================================================
// ROUTES EXPRESS (Webhook, Stats, Admin, Dashboard, etc.)
// ============================================================
...
app.post('/admin/generate-code', async (req, res) => {
  ...
  if (!process.env.ADMIN_PASSWORD) return res.json({ success: false, erreur: 'ADMIN_PASSWORD n\'est pas configuré sur le serveur.' });
  if (motDePasse !== process.env.ADMIN_PASSWORD) return res.json({ success: false, erreur: 'Mot de passe incorrect.' });
  const creditsNum = parseInt(credits, 10);
  if (!creditsNum || creditsNum <= 0) return res.json({ success: false, erreur: 'Nombre de crédits invalide.' });
  const code = (codePerso && codePerso.trim()) ? codePerso.trim().toUpperCase() : genererCodeAleatoire();
  if (await codeDejaUtilise(code)) return res.json({ success: false, erreur: 'Ce code existe déjà et a été utilisé.' });
...
<style>
  * { box-sizing: border-box; }
  ...
  .cartes { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 12px; margin-bottom: 24px; }
  .carte { background: white; border-radius: 12px; padding: 16px; box-shadow: 0 1px 4px rgba(0,0,0,0.08); text-align: center; }
  .carte .valeur { font-size: 28px; font-weight: 700; color: #2563eb; }
  .carte .label { font-size: 12px; color: #666; margin-top: 4px; }
  .bloc-graphique { background: white; border-radius: 12px; padding: 16px; box-shadow: 0 1px 4px rgba(0,0,0,0.08); }
  .bloc-graphique h2 { font-size: 15px; margin: 0 0 12px; }
  .actualiser { display: inline-block; margin-top: 16px; font-size: 13px; color: #2563eb; cursor: pointer; text-decoration: underline; }
...
async function charger() {
  ...
  const data = await res.json();
  document.getElementById('sousTitre').textContent = 'Journée du ' + data.date + ' — quota gratuit estimé : ' + data.quotaGratuitEstimeParJour + ' requêtes (' + data.nombreDeClesConfigurees + ' clé(s) configurée(s))';
  const restant = Math.max(data.quotaGratuitEstimeParJour - data.totalAppelsGemini, 0);
  const pourcentage = data.quotaGratuitEstimeParJour > 0 ? Math.round((data.totalAppelsGemini / data.quotaGratuitEstimeParJour) * 100) : 0;
  ...
  if (entrees.length === 0) { canvas.style.display = 'none'; videMessage.style.display = 'block'; return; }
  canvas.style.display = 'block'; videMessage.style.display = 'none';
  const labels = entrees.map(([k]) => k);
  const valeurs = entrees.map(([, v]) => v);
  if (graphiqueActuel) graphiqueActuel.destroy();
  graphiqueActuel = new Chart(canvas, { type: 'bar', data: { labels, datasets: [{ label: 'Appels', data: valeurs, backgroundColor: '#2563eb', borderRadius: 6 }] }, options: { responsive: true, plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { precision: 0 } } } } });
}
charger();
...
// ============================================================
// RECEPTION DES MESSAGES (Webhook POST)
// ============================================================
app.post('/webhook', async (req, res) => {
  ...
  if (body.object === 'page') {
    ...
    for (const entry of body.entry) {
      ...
      if (imageAttachment) {
        handleImageEvent(senderId, imageAttachment.payload.url).catch(err => console.error('Erreur handleImageEvent:', err));
      } else if (audioAttachment) {
        handleAudioEvent(senderId, audioAttachment.payload.url).catch(err => console.error('Erreur handleAudioEvent:', err));
      } else if (event.message && event.message.text) {
        const payload = event.message.quick_reply?.payload;
        const userText = event.message.text.trim();
        handleEvent(senderId, payload || userText, !!payload).catch(err => console.error('Erreur handleEvent:', err));
      }
      if (event.postback && event.postback.payload) {
        handleEvent(senderId, event.postback.payload, true).catch(err => console.error('Erreur handleEvent (postback):', err));
      }
    }
  } else {
    res.sendStatus(404);
...
// ============================================================
// LE ROUTEUR PRINCIPAL (handleEvent) – MODIFIÉ AVEC XP ET PROFIL
// ============================================================
const userModes = {};
...
async function handleEvent(senderId, texteOuPayload, estUnBouton) {
  ...
  if (peutChangerDeModeParMotCle) {
    // --- Gestion des changements de mode (boutons / mots-clés) ---
    ...
    if (texteOuPayload === 'CHAT_IA' || MOTS_CLES_CHAT_IA.test(texteOuPayload)) {
      ...
      await sendMessage(senderId, '🤖 Tu discutes avec l\'IA. Pose-moi tes questions !', BOUTON_MENU); return;
    }
    ...
    if (texteOuPayload.startsWith('HIANATRA_AUDIO_')) {
      ...
      try {
        ...
        await axios.post(`https://graph.facebook.com/v21.0/me/messages?access_token=${PAGE_ACCESS_TOKEN}`, {
          ...
          message: { attachment: { type: 'audio', payload: { url: publicUrl, is_reusable: true } } }
        });
      } catch (err) { console.error('Erreur TTS Proxy:', err.message); await sendTyping(senderId, false); await sendMessage(senderId, "❌ Impossible de générer l'audio pour le moment. Réessaie dans quelques instants."); }
      return;
    }
    ...
    if (texteOuPayload === 'MENU_CORRECTION_EXERCICES' || MOTS_CLES_CORRECTION_EXERCICES.test(texteOuPayload)) {
      ...
      await sendMessage(senderId, '🖊️ Mode Correction d\'exercices activé (toutes matières).\n\nEnvoie-moi le texte de l\'exercice/devoir/sujet, (ou directement une 📷 photo de la fiche), et je te donne le corrigé complet.', BOUTON_MENU); return;
    }
    ...
    if (texteOuPayload === 'MENU_BAC' || MOTS_CLES_BAC.test(texteOuPayload)) {
      ...
      await sendMessage(senderId, `🧮 Simulateur Bac Madagascar\n\nQuelle est ta série ? (${Object.keys(COEFFICIENTS_BAC).join(', ')})`, BOUTON_MENU); return;
    }
  ...
  switch (etat.mode) {
    case 'resultats_menu': {
      ...
      if (choix === 'EXAM_CEPE' || choix === 'CEPE') { userModes[senderId] = { mode: 'resultats', typeExam: 'cepe' }; await sendMessage(senderId, `🎓 Mode Résultats CEPE activé.\n\nAlefaso eto ny n°matricule na anarana feno.`, BOUTON_MENU); }
      else if (choix === 'EXAM_BEPC' || choix === 'BEPC') { userModes[senderId] = { mode: 'resultats', typeExam: 'bepc' }; await sendMessage(senderId, `🎓 Mode Résultats BEPC activé.\n\nAlefaso eto ny n°matricule na anarana feno.`, BOUTON_MENU); }
      else if (choix === 'EXAM_BACC' || choix === 'BACC') { userModes[senderId] = { mode: 'choix_province_bacc' }; await sendMessage(senderId, '🎓 Résultats BACC\n\nChoisis ou tape le nom de ta province (ex: Antananarivo, Fianarantsoa, Toamasina, Mahajanga, Toliara, Antsiranana) :',
      ...
      else await sendMessage(senderId, "❌ Choix non reconnu. Tape CEPE, BEPC ou BACC :");
      return;
    }
    case 'choix_province_bacc': {
      ...
      if (province) { userModes[senderId] = { mode: 'resultats_bacc', province }; await sendMessage(senderId, `🎓 Résultats BACC - Province : ${province.toUpperCase()}\n\nAlefaso eto ny n° d\'inscription (7 chiffres) na anarana feno.`, BOUTON_MENU); }
      else await sendMessage(senderId, "❌ Province non reconnue. Tape le nom exact (ex: Antananarivo, Fianarantsoa, Toamasina, Mahajanga, Toliara, Antsiranana) :");
      return;
    }
    case 'resultats_bacc': {
      await sendTyping(senderId, true);
      ...
      } else { await sendMessage(senderId, res, BOUTON_MENU); await ajouterXP(senderId, 10, 'resultat_bac'); }
      return;
    }
    case 'admin_identifiant': {
      ...
      await sendMessage(senderId, '🔐 Mot de passe :'); return;
    }
    case 'admin_motdepasse': {
      ...
      if (!identifiantOk || !motDePasseOk) { userModes[senderId] = { mode: 'chat' }; await sendMessage(senderId, '❌ Identifiant ou mot de passe incorrect.'); return; }
    ...
    case 'admin_menu': {
      if (MOTS_CLES_QUITTER_ADMIN.test(texteOuPayload)) { userModes[senderId] = { mode: 'chat' }; return envoyerMenu(senderId); }
      if (/^code$/i.test(texteOuPayload.trim())) { userModes[senderId] = { mode: 'admin_code_credits' }; await sendMessage(senderId, '💳 Combien de crédits pour ce code ?'); return; }
      if (/^alerte$/i.test(texteOuPayload.trim())) { await sendMessage(senderId, '🔔 Quelle province vient de sortir ses résultats ?', [
      ...
      if (texteOuPayload.startsWith('ADMIN_ALERTE_')) { const province = texteOuPayload.replace('ADMIN_ALERTE_', ''); userModes[senderId] = { mode: 'admin_confirmation_alerte', provinceAlerte: province }; await sendMessage(senderId, `⚠️ Envoyer les alertes pour **${province}** ? (tape "OUI" pour confirmer)`); return; }
    ...
    case 'admin_code_credits': {
      ...
      if (!creditsNum || creditsNum <= 0) { await sendMessage(senderId, 'Nombre invalide. Combien de crédits pour ce code ?'); return; }
    ...
    case 'admin_confirmation_alerte': {
      if (/^oui$/i.test(texteOuPayload.trim())) { await sendMessage(senderId, '🚀 Envoi des alertes...'); const nb = await declencherAlertes(etat.provinceAlerte); userModes[senderId] = { mode: 'admin_menu' }; await sendMessage(senderId, `✅ Terminé ! ${nb} alertes envoyées.`); }
      else { userModes[senderId] = { mode: 'admin_menu' }; await sendMessage(senderId, '❌ Annulé.'); }
      return;
    }
    case 'admin_code_perso': {
      ...
      if (await codeDejaUtilise(code)) { userModes[senderId] = { mode: 'admin_menu' }; await sendMessage(senderId, `⚠️ "${code}" existe déjà et a été utilisé. Tape "code" pour réessayer.`); return; }
    ...
    case 'simulation_bac_serie': {
      ...
      if (!serie) { await sendMessage(senderId, `Série non reconnue. Choisis parmi : ${Object.keys(COEFFICIENTS_BAC).join(', ')}`); return; }
    ...
    case 'simulation_bac_notes': {
      ...
      if (isNaN(note) || note < 0 || note > 20) { await sendMessage(senderId, `Note invalide. Donne une note entre 0 et 20 pour ${matiereActuelle} :`); return; }
      etat.notes[matiereActuelle] = note;
      const indexSuivant = etat.index + 1;
      if (indexSuivant < etat.matieres.length) { userModes[senderId] = { mode: 'simulation_bac_notes', serie: etat.serie, matieres: etat.matieres, index: indexSuivant, notes: etat.notes }; await sendMessage(senderId, `Note en ${etat.matieres[indexSuivant]} (/20) ?`); return; }
    ...
    case 'creation_cv': {
      ...
      if (etapeSuivanteIndex < ETAPES_CV.length) { userModes[senderId] = { mode: 'creation_cv', etapeIndex: etapeSuivanteIndex, donnees: etat.donnees }; await sendMessage(senderId, ETAPES_CV[etapeSuivanteIndex].question, BOUTON_MENU); return; }
    ...
    case 'creation_cv_loisirs_photo': {
      ...
      if (/^passe$/i.test(texteOuPayload.trim())) { await genererEtEnvoyerCv(senderId, etat.donnees, null); await ajouterXP(senderId, 20, 'cv_creation'); return; }
    ...
    case 'attente_code': {
      ...
      if (!creditsDuCode) { await sendMessage(senderId, '❌ Ce code n\'est pas valide. Vérifie qu\'il est bien écrit, ou contacte Tsarafandray Services pour en obtenir un.', BOUTON_MENU); return; }
      if (await codeDejaUtilise(code)) { await sendMessage(senderId, '⚠️ Ce code a déjà été utilisé.', BOUTON_MENU); return; }
    ...
    case 'traduction': {
      if (!etat.langue) { userModes[senderId] = { mode: 'traduction', langue: texteOuPayload }; await sendMessage(senderId, `Ok, envoie-moi tes textes, je les traduis en ${texteOuPayload}.`, BOUTON_MENU); return; }
      await sendTyping(senderId, true);
    ...
    case 'correction_exercices': {
      const acces = await verifierEtConsommerCredit(senderId);
      if (!acces.autorise) { await sendMessage(senderId, `🔒 Tu as utilisé tes ${LIMITE_GRATUITE_PAR_JOUR} corrections gratuites d'aujourd'hui, et tu n'as plus de crédits.\n\nRevien demain pour de nouvelles corrections gratuites, ou tape "code" pour activer des crédits supplémentaires.`, BOUTON_MENU); return; }
      await sendTyping(senderId, true);
      const profile = await getProfile(senderId);
      ...
      if (MOTS_CLES_GRAPHIQUE.test(texteOuPayload)) {
        ...
        if (donnees) { const urlGraphique = await genererGraphiqueMath(donnees.formule, donnees.xMin, donnees.xMax); if (urlGraphique) await sendImage(senderId, urlGraphique); }
      }
      return;
    }
    case 'exercices': {
    ...
    case 'hianatra_menu': {
      ...
      const choix = texteOuPayload.toUpperCase().trim();
      if (choix === 'HIANATRA_INFO' || choix === '1' || choix === 'INFORMATIQUE' || choix === 'INFO') { discipline = 'Informatique'; instruction = 'Tu es un expert en informatique. Aide l\'utilisateur à apprendre la programmation, la bureautique ou la technologie. Sois pédagogique et donne des exemples concrets.'; }
      else if (choix === 'HIANATRA_LANGUES' || choix === '2' || choix === 'LANGUES' || choix === 'LANGUE') { discipline = 'Langues'; instruction = 'Tu es un tuteur de langues expert (Français, Anglais, Malagasy). Aide l\'utilisateur à pratiquer. Propose des exercices de traduction ou de conversation. Si l\'utilisateur écrit en Malagasy, réponds en Malagasy et en Français/Anglais pour l\'aider à apprendre.'; }
      else if (choix === 'HIANATRA_LECONS' || choix === '3' || choix === 'LEÇONS' || choix === 'LECONS' || choix === 'LEÇON' || choix === 'LECON') { discipline = 'Leçons Scolaires'; instruction = 'Tu es un professeur polyvalent. Aide l\'utilisateur avec ses cours (Maths, SVT, Histoire, etc.). Explique les concepts complexes simplement.'; }
      else { await sendMessage(senderId, "❌ Choix non reconnu. Tape 1, 2 ou 3 :\n1️⃣ Informatique\n2️⃣ Langues\n3️⃣ Leçons"); return; }
    ...
    case 'hianatra_session': {
      ...
      } catch (err) { console.error('Erreur hianatra_session:', err.message); await sendTyping(senderId, false); await sendMessage(senderId, "❌ Une petite erreur est survenue dans ton cours. Réessaie !", BOUTON_MENU); }
      return;
    }
    default: {
      await sendTyping(senderId, true);
...
async function handleImageEvent(senderId, imageUrl) {
  ...
  if (etat.mode === 'creation_cv') {
    ...
    } catch (err) { console.error('Erreur extraction CV image:', err.response?.data || err.message); await sendTyping(senderId, false); await sendMessage(senderId, `Désolé, je n'ai pas réussi à lire cette image. On continue avec les questions :\n\n${ETAPES_CV[etat.etapeIndex].question}`, BOUTON_MENU); }
    return;
  }
  if (etat.mode === 'creation_cv_loisirs_photo' && etat.etapePhoto === true) {
    ...
    } catch (err) { console.error('Erreur réception photo CV:', err.message); await sendMessage(senderId, "Désolé, je n'ai pas réussi à récupérer cette photo. Tape \"passe\" pour continuer sans photo, ou renvoie une image.", BOUTON_MENU); }
    return;
  }
...
// ============================================================
// GESTION DES MESSAGES VOCAUX (handleAudioEvent)
// ============================================================
async function handleAudioEvent(senderId, audioUrl) {
  ...
  try {
    ...
    } else { await sendMessage(senderId, `🎓🎙️ ${reponse}`, BOUTON_MENU); }
    await ajouterXP(senderId, 5, 'hianatra_audio');
  } catch (err) { console.error('Erreur handleAudioEvent:', err.message); await sendTyping(senderId, false); await sendMessage(senderId, "❌ Désolé, je n'ai pas réussi à analyser ton message vocal. Assure-toi qu'il est clair et réessaie.", BOUTON_MENU); }
}
